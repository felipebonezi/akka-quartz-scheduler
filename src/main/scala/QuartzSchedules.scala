...
object QuartzSchedules {
  ...
  def apply(config: Config, defaultTimezone: TimeZone): immutable.Map[String, QuartzSchedule] =
    catchMissing opt {
  ...
  def parseCronSchedule(name: String, desc: Option[String], config: Config)(
      tz: TimeZone,
    ...
    val expression = catchMissing or catchWrongType either config.getString("expression") match {
      ...
      case Right(str) =>
        catchParseErr either new CronExpression(str) match {
          case Left(t) =>
            throw new IllegalArgumentException(
              "Invalid 'expression' for Cron Schedule '%s'. Failed to validate CronExpression."
                .format(name),
              t
            )
