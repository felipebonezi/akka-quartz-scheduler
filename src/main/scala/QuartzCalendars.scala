...
object QuartzCalendars {
  ...
  def apply(config: Config, defaultTimezone: TimeZone): immutable.Map[String, Calendar] =
    catchMissing opt {
  ...
  def parseAnnualCalendar(name: String, config: Config)(tz: TimeZone): AnnualCalendar = {
    ...
    val excludeDates =
      catchMissing or catchWrongType either config.getStringList("excludeDates") match {
        case Left(t) =>
  ...
  def parseDailyCalendar(name: String, config: Config): DailyCalendar = {
    def parseTimeEntry(entry: String) =
      catchMissing or catchWrongType either config.getString(entry) match {
        case Left(t) =>
          throw new IllegalArgumentException(
  ...
  def parseWeeklyCalendar(name: String, config: Config): WeeklyCalendar = {
    ...
    require(
      ...
      "Weekly Calendar '%s' - 'excludeDays' must consist of a list of Integers between 1 and 7"
        .format(name)
    )
  ...
  def parseMonthlyCalendar(name: String, config: Config): MonthlyCalendar = {
    ...
    require(
      ...
      "Monthly Calendar '%s' - 'excludeDays' must consist of a list of Integers between 1 and 31"
        .format(name)
    )
  ...
  def parseCronCalendar(name: String, config: Config): CronCalendar = {
    val exclude =
      catchMissing or catchWrongType either config.getString("excludeExpression") match {
        case Left(t) =>
          throw new IllegalArgumentException(
        ...
        case Right(expr) =>
          catchParseErr either CronExpression.validateExpression(expr) match {
            case Left(t) =>
              throw new IllegalArgumentException(
                "Invalid 'excludeExpression' for Cron Calendar '%s'. Failed to validate CronExpression."
                  .format(name),
                t
              )
            case Right(_) => expr
